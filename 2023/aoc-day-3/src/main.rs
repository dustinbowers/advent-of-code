fn main() {
    // Parse input into a 2d matrix of chars
    let mut field: Vec<Vec<char>> = vec![];
    let lines: Vec<String> = INPUT.lines().map(String::from).collect();
    for l in &lines {
        field.push(l.chars().collect());
    }

    // Initialize another identically sized 2D field
    let mut filtered_field: Vec<Vec<char>> = vec![];
    for i in 0..lines.len() {
        let mut line_chars: Vec<char> = vec![];
        for _ in 0..=lines[i].len() {
            line_chars.push(' ');
        }
        filtered_field.push(line_chars);
    }
    ...
    for (i, l) in field.iter().enumerate() {
        for (j, c) in l.iter().enumerate() {
            ...
            match c {
                // '*' => {
                    '!' | '@' | '#' | '$' | '%' | '^' | '&' | '*' | '-' | '+' | '=' | '/' => {
                    filtered_field[i][j] = *c;
                    for r in (ii - 1)..=(ii + 1) {
                        if r < 0 || r >= filtered_field.len() as i32 {
                            continue;
                        }
                        for c in (jj - 1)..=(jj + 1) {
                            if c < 0 || c >= filtered_field[r as usize].len() as i32 {
                                continue;
                            }

                            let rr = r as usize;
                            let cc = c as usize;
                            // If we find a digit, keep adding leftward and rightwards
                            match field[rr][cc] {
                                '0'..='9' => {
                                    filtered_field[rr][cc] = field[rr][cc];
                                    let mut curr_c = c;
                                    // add leftward
                                    loop {
                                        let left_c = curr_c - 1;
                                        ...
                                        match field[rr][left_c as usize] {
                                            '0'..='9' => {
                                                filtered_field[rr][left_c as usize] =
                                                    field[rr][left_c as usize];
                                                curr_c -= 1;
                                            }
                                            _ => {
                                                break;
                                    ...
                                    let mut curr_c = c;
                                    loop {
                                        let right_c = curr_c + 1;
                                        if right_c as usize >= field[rr].len() {
                                            break;
                                        }
                                        match field[rr][right_c as usize] {
                                            '0'..='9' => {
                                                filtered_field[rr][right_c as usize] =
                                                    field[rr][right_c as usize];
                                                curr_c += 1;
                                            }
                                            _ => {
                                                break;
    ...
    let mut total = 0;
    for (_, l) in filtered_field.iter().enumerate() {
        let line: String = l.into_iter().collect();
        println!("Line: {}", line);
        let parts: Vec<&str> = line.split(" ").collect();
        for (_, n) in parts.iter().enumerate() {
            let r = n.parse::<i32>();
            match r {
                Ok(n) => {
                    total += n;
    ...
    let mut gear_total : i32 = 0;
    let num_rows = filtered_field.len();
    let num_cols = filtered_field[0].len();
    let filtered_field_copy = filtered_field.clone();
    for i in 0..num_rows {
        for j in 0..num_cols {
            let filtered_field = filtered_field_copy.clone();
            let c = filtered_field[i][j];
    // for (i, l) in filtered_field.iter_mut().enumerate() {
    //     for (j, c) in l.iter_mut().enumerate() {
            ...
            match c {
                '*' => {
                    let mut gear_nums: Vec<String> = vec![];
                    // loop around the gear
                    for r in (ii - 1)..=(ii + 1) {
                        // let mut field_len: usize  = l.len();
                        if r < 0 || r >= num_rows as i32 {
                            continue;
                        }
                        for c in (jj - 1)..=(jj + 1) {
                            // if ii == jj || c < 0 || c >= filtered_field[r as usize].len() as i32 {
                            if ii == jj || c < 0 || c >= num_cols as i32 {
                                continue;
                            }

                            let rr = r as usize;
                            let cc = c as usize;
                            let mut curr_num_str;
                            // If we find a digit, keep adding leftward and rightwards
                            let current_filtered_field : char = format!("{}", filtered_field[rr][cc].to_owned()).chars().nth(0).unwrap();
                            match current_filtered_field { //filtered_field[rr][cc] {
                                '0'..='9' => {
                                    let mut curr_c = c;
                                    curr_num_str = format!("{}", filtered_field[rr][cc]);
                                    // filtered_field[rr][cc] = '-';
                                    // add leftward
                                    loop {
                                        let left_c = curr_c - 1;
                                        if left_c < 0 {
                                        ...
                                        match field[rr][left_c as usize] {
                                            '0'..='9' => {
                                                ...
                                                //     field[rr][left_c as usize];
                                                curr_c -= 1;
                                                curr_num_str = format!("{}{}", filtered_field[rr][left_c as usize], curr_num_str);
                                                filtered_field[rr][left_c as usize] = '-';
                                            }
                                            _ => {
                                                break;
                                    ...
                                    loop {
                                        ...
                                        match field[rr][right_c as usize] {
                                            '0'..='9' => {
                                                ...
                                                //     field[rr][right_c as usize];
                                                curr_c += 1;
                                                curr_num_str = format!("{}{}", curr_num_str, filtered_field[rr][right_c as usize]);
                                                filtered_field[rr][right_c as usize] = '-';
                                            }
                                            _ => {
                                                break;
                                    ...
                                    gear_nums.push(curr_num_str);
                                }
                                _ => {}
                            }
                        }
                    }
                    println!("gear_nums = {:?}", gear_nums);
                    if gear_nums.len() == 2 {
                        let num1 = gear_nums[0].parse::<i32>().unwrap();
                        let num2 = gear_nums[1].parse::<i32>().unwrap();
                        println!("Adding gear at ({},{}): Gear1 = {}, Gear2 = {}", i, j, num1, num2);
                        gear_total += num1 * num2;
                    }
                },
                _ => {},
            }
